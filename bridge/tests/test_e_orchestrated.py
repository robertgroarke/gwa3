...
from .dungeon_movement import push_until_map_changes, wait_until_near
from ..farming_knowledge import DUNGEONS, MAP_NAMES

MAP_GADDS = DUNGEONS["Bogroot Growths"]["entry_outpost_map_id"]
MAP_SPARKFLY = next(
    map_id for map_id, name in MAP_NAMES.items() if name == "Sparkfly Swamp"
)
...
async def test_orchestrated_phase3_merchant(tc: BridgeTestCase):
    ...
    tc.assert_action_success(result)
    reached = await wait_until_near(tc, MERCHANT_X, MERCHANT_Y, threshold=350.0, timeout=15.0)
    ...
    await wait_until_near(tc, npc_x, npc_y, threshold=150.0, timeout=10.0)
...
async def test_orchestrated_phase4_explorable(tc: BridgeTestCase):
    ...
    for wx, wy in [EXIT_WP1, EXIT_WP2]:
        ...
        reached = await wait_until_near(tc, wx, wy, threshold=350.0, timeout=25.0)
        if not reached:
            tc.skip(f"Failed to reach exit waypoint ({wx}, {wy})")
            return

    # Push toward zone exit — keep moving until map changes
    left_outpost = await push_until_map_changes(
        tc,
        EXIT_PUSH[0],
        EXIT_PUSH[1],
        MAP_GADDS,
    ...
    if not foes:
        ...
        reached = await wait_until_near(tc, -4559.0, -14406.0, threshold=500.0, timeout=15.0)
        snap2 = await tc.wait_for_snapshot(tier=2, timeout=5.0)
        foes = [a for a in snap2.get("agents", [])
                if a.get("allegiance") == 3 and a.get("is_alive", False)]
