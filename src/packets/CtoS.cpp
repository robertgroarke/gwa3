...
bool Initialize() {
    ...
    // Immediate smoke test: send ACTION_CANCEL (1 DWORD, no side effects)
    Log::Info("CtoS: [SMOKE] Calling PacketSend with ACTION_CANCEL...");
    uint32_t smokeData[1] = { Packets::ACTION_CANCEL };
    s_packetSendFn(reinterpret_cast<void*>(s_packetLocation), 4, smokeData);
    Log::Info("CtoS: [SMOKE] PacketSend returned OK!");
