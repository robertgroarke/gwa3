...
// Pointers used by the dispatch path are set during Initialize from Offsets.
...
// Avoid C++ calls on the engine hook hot path; call through cached pointers.
...
// Statically allocated because the engine detour is single-threaded per process.
...
static __declspec(naked) void EngineDetourNaked() {
    __asm {
        ...
        // The exit path pushes onto the FPU stack; corrupted FPU state can crash GW.
    ...
    no_botshub_command:
        ...
        // Game command dispatch for rare commands such as salvage.
...
int32_t GetBotshubQueuePending() {
    const LONG pending = s_botshubCmdHead - s_botshubCmdTail;
    return pending > 0 ? static_cast<int32_t>(pending) : 0;
}
...
static uintptr_t ResolvePacketLocation() {
    ...
    if (ptrAddr > 0x10000) {
        __try {
            ...
            Log::Warn("CtoS: PacketLocation dereference failed at 0x%08X",
                      static_cast<unsigned>(ptrAddr));
        }
    }
...
void SendPacket(uint32_t size, uint32_t header, ...) {
    ...
    if (GameThread::IsInitialized()) {
        ...
        // 7 heroes all casting (each producing a 0x46 USE_SKILL packet),
...
void HeroAdd(uint32_t heroId) {
    if (SendPacketBotshub(2, Packets::HERO_ADD, heroId)) {
        return;
    }
    Log::Warn("CtoS: HeroAdd falling back to direct packet path hero=%u", heroId);
    SendPacketDirect(2, Packets::HERO_ADD, heroId);
}

void HeroKick(uint32_t heroId) {
    if (SendPacketBotshub(2, Packets::HERO_KICK, heroId)) {
        return;
    }
    Log::Warn("CtoS: HeroKick falling back to direct packet path hero=%u", heroId);
    SendPacketDirect(2, Packets::HERO_KICK, heroId);
}

void HeroBehavior(uint32_t heroIndex, uint32_t behavior) {
    if (SendPacketBotshub(3, Packets::HERO_BEHAVIOR, heroIndex, behavior)) {
        return;
    }
    Log::Warn("CtoS: HeroBehavior falling back to direct packet path hero=%u behavior=%u",
              heroIndex,
              behavior);
    SendPacketDirect(3, Packets::HERO_BEHAVIOR, heroIndex, behavior);
}

void ChangeSecondProfession(uint32_t agentId, uint32_t profession) {
    SendPacket(3, Packets::PROFESSION_CHANGE, agentId, profession);
}
...
void HeroFlagSingle(uint32_t heroIndex, float x, float y) {
    ...
    memcpy(&ix, &x, 4);
    memcpy(&iy, &y, 4);
    if (SendPacketBotshub(5, Packets::HERO_FLAG_SINGLE, heroIndex, ix, iy, 0u)) {
        return;
    }
    Log::Warn("CtoS: HeroFlagSingle falling back to direct packet path hero=%u",
              heroIndex);
    SendPacketDirect(5, Packets::HERO_FLAG_SINGLE, heroIndex, ix, iy, 0u);
}
...
void HeroFlagAll(float x, float y) {
    ...
    memcpy(&ix, &x, 4);
    memcpy(&iy, &y, 4);
    if (SendPacketBotshub(4, Packets::HERO_FLAG_ALL, ix, iy, 0u)) {
        return;
    }
    Log::Warn("CtoS: HeroFlagAll falling back to direct packet path");
    SendPacketDirect(4, Packets::HERO_FLAG_ALL, ix, iy, 0u);
}
...
void UseHeroSkill(uint32_t heroAgentId, uint32_t skillId, uint32_t targetAgentId) {
    // Legacy GWA2 sends HERO_USE_SKILL as hero agent id + resolved skill id,
    // not hero party slot + skillbar slot.
    Log::Info("CtoS: UseHeroSkill packet heroAgent=%u skillId=%u target=%u via=pre-dispatch",
              heroAgentId,
              skillId,
              targetAgentId);
    SendPacket(5, Packets::USE_HERO_SKILL, heroAgentId, skillId, 0u, targetAgentId);
}
