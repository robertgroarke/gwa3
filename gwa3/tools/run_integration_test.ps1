...
$script:IntegrationFlagPath = Join-Path $script:BinDir "gwa3_test_integration.flag"
...
function Capture-RunScreenshot {
    ...
    $path = Join-Path $script:ScreenshotDir ("integration_{0}_{1}.png" -f $Tag, $stamp)
    & $script:CaptureScript -OutputPath $path | Out-Null
    return $path
}

function Get-LatestIntegrationBlock {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "=== GWA3 Integration Test Suite ===") {
            $start = $i
        }
    }
    if ($start -lt 0) {
...
New-Item -ItemType File -Force -Path $script:IntegrationFlagPath | Out-Null

Launch-GwAccount
$gwPid = Wait-ForGwWindow -TimeoutSeconds $LaunchTimeoutSeconds

Write-Host "Injecting integration test into PID $gwPid..."
& $script:InjectorPath --pid $gwPid --test-integ
...
while (((Get-Date) - $startTime).TotalSeconds -lt $RunTimeoutSeconds) {
    ...
    if (Test-Path $script:LogPath) {
        $integrationLines = Get-LatestIntegrationBlock
        foreach ($line in $integrationLines) {
            ...
            if ($line -match "Integration test complete: .* failures") {
                $failureLine = $line
            }
        }
    }
...
$integrationBlock = @()
for ($i = 0; $i -lt 5; $i++) {
    $integrationBlock = Get-LatestIntegrationBlock
    if ($integrationBlock.Count -gt 0) {
        break
    }
    Start-Sleep -Seconds 1
}
if ($integrationBlock.Count -eq 0) {
    Write-Host "No integration block found in log."
    exit 2
}

foreach ($line in $integrationBlock) {
    if ($line -match "\[WATCHDOG\].*DISCONNECT DETECTED") {
        $disconnectSeen = $true
    }
    if ($line -match "=== SUMMARY:") {
...
$integrationBlock | ForEach-Object { $_ }

if ($crashDialogSeen) {
    Write-Error "Integration run failed: visible Gw.exe crash dialog detected."
    exit 10
}

if ($disconnectSeen) {
    Write-Error "Integration run failed: disconnect detected by watchdog."
    exit 11
}

if (-not $summarySeen) {
    Write-Error "Integration run failed: no summary line found; run is not trustworthy."
    exit 12
}

if ($failureLine -and $failureLine -match "Integration test complete: (\d+) failures") {
