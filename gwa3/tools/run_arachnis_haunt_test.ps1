...
$defaultBuildDir = Join-Path $script:RepoRoot "build_trade"
...
if (-not $DllName) {
    $tradeDll = Join-Path $resolvedBuildDir "bin\Release\gwa3_trade.dll"
    if (Test-Path $tradeDll) {
        $defaultDllName = "gwa3_trade.dll"
    }
}
...
$script:ArachnisFlagPath = Join-Path $script:BinDir "gwa3_test_arachnis.flag"
...
public static class GwWindowProbeArachnis {
...
function Get-GwCrashDialog {
    ...
    $found = $false
    $callback = [GwWindowProbeArachnis+EnumWindowsProc]{
        param([IntPtr]$hWnd, [IntPtr]$lParam)
        if (-not [GwWindowProbeArachnis]::IsWindowVisible($hWnd)) { return $true }
        $cls = New-Object System.Text.StringBuilder 256
        [void][GwWindowProbeArachnis]::GetClassName($hWnd, $cls, $cls.Capacity)
        $ttl = New-Object System.Text.StringBuilder 512
        [void][GwWindowProbeArachnis]::GetWindowText($hWnd, $ttl, $ttl.Capacity)
        if ($cls.ToString() -eq "#32770" -and $ttl.ToString() -eq "Gw.exe") {
            $windowPid = 0
            [void][GwWindowProbeArachnis]::GetWindowThreadProcessId($hWnd, [ref]$windowPid)
    ...
    [void][GwWindowProbeArachnis]::EnumWindows($callback, [IntPtr]::Zero)
    return $script:found
}
...
function Capture-RunScreenshot {
    ...
    $path = Join-Path $script:ScreenshotDir ("arachnis_{0}_{1}.png" -f $Tag, $stamp)
    & $script:CaptureScript -OutputPath $path | Out-Null
    return $path
}

function Get-LatestArachnisBlock {
    if (-not (Test-Path $script:LogPath)) { return @() }
    try { $lines = Get-Content $script:LogPath -ErrorAction Stop } catch { return @() }
    $start = -1
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "ARACHNIS HAUNT FEATURE TEST MODE") { $start = $i }
    }
...
New-Item -ItemType File -Force -Path $script:ArachnisFlagPath | Out-Null
...
if (-not (Test-UsableGwPid -ProcessId $gwPid)) {
    ...
    Write-Host "Using launcher PID for Arachnis test: $gwPid"
}
...
Write-Host "Injecting Arachnis Haunt test into PID $gwPid (DLL: $dllName)..."
& $script:InjectorPath --pid $gwPid --dll $dllName --test-arachnis
...
while (((Get-Date) - $startTime).TotalSeconds -lt $RunTimeoutSeconds) {
    ...
    if (Test-Path $script:LogPath) {
        $arachnisLines = Get-LatestArachnisBlock
        foreach ($line in $arachnisLines) {
            if ($line -match "\[WATCHDOG\].*(CRASH DIALOG|WINDOW NOT RESPONDING|RENDER FROZEN)") {
                $crashDialogSeen = $true
            }
            if ($line -match "ARACHNIS HAUNT FEATURE TEST COMPLETE") {
                $summarySeen = $true
                if (-not $summarySeenAt) { $summarySeenAt = Get-Date }
            }
            if ($line -match "Arachnis Haunt feature test complete: .* failures") {
                $failureLine = $line
            }
        }
...
$arachnisBlock = @()
for ($i = 0; $i -lt 5; $i++) {
    $arachnisBlock = Get-LatestArachnisBlock
    if ($arachnisBlock.Count -gt 0) { break }
    Start-Sleep -Seconds 1
}

if ($arachnisBlock.Count -eq 0) {
    Write-Host "No Arachnis Haunt test block found in log."
    exit 2
}

$arachnisBlock | ForEach-Object { $_ }

foreach ($line in $arachnisBlock) {
    if ($line -match "\[WATCHDOG\].*(CRASH DIALOG|WINDOW NOT RESPONDING|RENDER FROZEN)") {
        $crashDialogSeen = $true
    }
    if ($line -match "ARACHNIS HAUNT FEATURE TEST COMPLETE") {
    ...
    if ($line -match "Arachnis Haunt feature test complete: (\d+) failures") {
        $failureLine = $line
    }
}

if ($crashDialogSeen) {
    Write-Error "Arachnis Haunt run failed: visible Gw.exe crash dialog detected."
    exit 10
}

if (-not $summarySeen) {
    Write-Error "Arachnis Haunt run failed: no summary line found."
    exit 12
}

if ($failureLine -and $failureLine -match "Arachnis Haunt feature test complete: (\d+) failures") {
