...
constexpr uint32_t MAP_LONGEYES_LEDGE = 650u;
constexpr uint32_t QUEST_VEILED_THREAT = 803u;
constexpr uint32_t DIALOG_NPC_BODY = 0x8101u;
constexpr uint32_t DIALOG_COMPLETE_OLD_QUEST = 0x832307u;
constexpr uint32_t DIALOG_PICK_VEILED_THREAT = 0x832303u;
constexpr uint32_t DIALOG_ACCEPT_VEILED_THREAT = 0x832301u;
...
bool ReplayCheckpointBacktrack(const RouteDefinition& route, int currentIndex, int backtrackStart) {
    for (int j = currentIndex - 1; j >= backtrackStart; --j) {
        if (!MoveToWaypoint(route.waypoints[j], route.map_id)) {
            LogBot("Rragars: failed checkpoint backtrack at waypoint %d (%s) on %s",
                   j, route.waypoints[j].label, route.name);
            return false;
        }
    }
    return true;
...
bool HasVeiledThreatQuest() {
    return QuestMgr::GetActiveQuestId() == QUEST_VEILED_THREAT
        || QuestMgr::GetQuestById(QUEST_VEILED_THREAT) != nullptr;
}
...
bool EnsureVeiledThreatQuest() {
    ...
    static constexpr uint32_t kCompleteDialogs[] = {
        DIALOG_NPC_BODY,
        DIALOG_COMPLETE_OLD_QUEST,
        DIALOG_NPC_BODY,
        DIALOG_COMPLETE_OLD_QUEST,
    };
    static constexpr uint32_t kAcceptDialogs[] = {
        DIALOG_NPC_BODY,
        DIALOG_PICK_VEILED_THREAT,
        DIALOG_NPC_BODY,
        DIALOG_ACCEPT_VEILED_THREAT,
    };

    LogBot("Rragars: acquiring Veiled Threat in Doomlore Shrine");
    if (!DungeonNavigation::MoveToAndWait(-17583.0f, 17668.0f, 250.0f, 20000u, 1000u, MAP_DOOMLORE_SHRINE).arrived ||
        !DungeonNavigation::MoveToAndWait(kQuestNpcX, kQuestNpcY, 250.0f, 20000u, 1000u, MAP_DOOMLORE_SHRINE).arrived) {
        return false;
    }
    ...
    MapMgr::Travel(MAP_LONGEYES_LEDGE);
    if (!DungeonNavigation::WaitForMapId(MAP_LONGEYES_LEDGE, 60000u)) {
        return false;
    }
    MapMgr::Travel(MAP_DOOMLORE_SHRINE);
    if (!DungeonNavigation::WaitForMapId(MAP_DOOMLORE_SHRINE, 60000u)) {
        return false;
    }

    if (!DungeonNavigation::MoveToAndWait(kQuestNpcX, kQuestNpcY, 250.0f, 20000u, 1000u, MAP_DOOMLORE_SHRINE).arrived) {
        return false;
    }
...
bool LeaveDoomloreForDalada() {
    ...
    if (!WaitForMapReady(MAP_DOOMLORE_SHRINE, 15000u) ||
        !DungeonNavigation::MoveToAndWait(-15024.0f, 16571.0f, 250.0f, 15000u, 1000u, MAP_DOOMLORE_SHRINE).arrived ||
        !DungeonNavigation::MoveToAndWait(-15968.0f, 14434.0f, 250.0f, 15000u, 1000u, MAP_DOOMLORE_SHRINE).arrived) {
        return false;
    }
    ...
    while ((GetTickCount() - start) < 60000u) {
        ...
        if (MapMgr::GetMapId() == MAP_DALADA_UPLANDS ||
            DungeonNavigation::WaitForMapId(MAP_DALADA_UPLANDS, 250u)) {
            return true;
        }
        WaitMs(250u);
    }
...
bool ExecuteRewardChestFlow() {
    ...
    while ((GetTickCount() - chestSearchStart) < 60000u) {
        ...
        if (!MoveToPoint(reward.staging_x, reward.staging_y, MAP_RRAGARS_MENAGERIE_LVL3, 300.0f, 10000u)) {
    ...
    if (!MoveToPoint(reward.chest_x, reward.chest_y, MAP_RRAGARS_MENAGERIE_LVL3, 250.0f, 15000u)) {
    ...
    if (MapMgr::GetMapId() == MAP_RRAGARS_MENAGERIE_LVL3) {
        MapMgr::Travel(MAP_DOOMLORE_SHRINE);
        if (!DungeonNavigation::WaitForMapId(MAP_DOOMLORE_SHRINE, 180000u) ||
            !WaitForMapReady(MAP_DOOMLORE_SHRINE, 15000u)) {
            LogBot("Rragars: failed returning to Doomlore Shrine after reward chest");
            return false;
        }
    }

    return MapMgr::GetMapId() == MAP_DOOMLORE_SHRINE;
}
    ...
    for (int i = startIndex; i < route.waypoint_count; ++i) {
        ...
        case WaypointBehavior::ValidateRetryCheckpoint: {
            ...
            case DungeonCheckpoint::CheckpointFailureAction::AbortRun:
                ...
                MapMgr::Travel(MAP_DOOMLORE_SHRINE);
                return DungeonNavigation::WaitForMapId(MAP_DOOMLORE_SHRINE, 60000u);
...
BotState HandleTownSetup(BotConfig&) {
    ...
    if (mapId == MAP_DOOMLORE_SHRINE || mapId == MAP_DALADA_UPLANDS || mapId == MAP_GROTHMAR_WARDOWNS || mapId == MAP_SACNOTH_VALLEY) {
...
BotState HandleTravel(BotConfig&) {
    ...
    if (!route) {
        if (mapId == MAP_RRAGARS_MENAGERIE_LVL1 || mapId == MAP_RRAGARS_MENAGERIE_LVL2 || mapId == MAP_RRAGARS_MENAGERIE_LVL3) {
    ...
    return MapMgr::GetMapId() == MAP_RRAGARS_MENAGERIE_LVL1 ? BotState::InDungeon : BotState::Traveling;
}
...
BotState HandleDungeon(BotConfig&) {
    ...
    switch (mapId) {
    case MAP_RRAGARS_MENAGERIE_LVL1:
        routeId = RouteId::Level1;
        break;
    case MAP_RRAGARS_MENAGERIE_LVL2:
        routeId = RouteId::Level2;
        break;
    case MAP_RRAGARS_MENAGERIE_LVL3:
    ...
    return MapMgr::GetMapId() == MAP_DOOMLORE_SHRINE ? BotState::InTown : BotState::InDungeon;
}
...
void Register() {
    ...
    cfg.hard_mode = true;
    cfg.target_map_id = MAP_RRAGARS_MENAGERIE_LVL1;
    cfg.outpost_map_id = MAP_DOOMLORE_SHRINE;
