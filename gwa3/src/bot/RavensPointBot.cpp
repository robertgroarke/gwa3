...
void LogQuestSnapshot(const char *context) {
  auto *quest = QuestMgr::GetQuestById(QUEST_RAVENS_POINT);
...
bool VerifyQuestState(bool expectPresent, const char *context,
  ...
  if (!DungeonQuestRuntime::WaitForQuestState(QUEST_RAVENS_POINT, expectPresent,
                                              options)) {
    LogBot("Ravens: quest 0x%X %s after %s", QUEST_RAVENS_POINT,
  ...
  if (expectPresent && requireReadyForEntry && !IsQuestReadyForEntry(quest)) {
    LogBot("Ravens: quest 0x%X present but not ready after %s (logState=%u)",
           QUEST_RAVENS_POINT, context, quest ? quest->log_state : 0u);
    return false;
  }
...
bool ExecuteQuestCycle() {
  ...
  if (IsQuestReadyForEntry(quest)) {
    ...
    if (QuestMgr::GetActiveQuestId() != QUEST_RAVENS_POINT) {
      QuestMgr::SetActiveQuest(QUEST_RAVENS_POINT);
      WaitMs(500u);
    }
...
bool ExecuteRewardChestFlow() {
  ...
  if (!MoveToTravelPoint(reward.staging_point, MAP_RAVENS_POINT_LVL3)) {
    return false;
  }
  ...
  return DungeonNavigation::WaitForMapId(MAP_VARAJAR_FELLS_1, 180000u);
}
...
BotState HandleTownSetup(BotConfig &cfg) {
  ...
  if (mapId == MAP_VARAJAR_FELLS_1) {
    return BotState::Traveling;
  }
  if (mapId == MAP_RAVENS_POINT_LVL1 || mapId == MAP_RAVENS_POINT_LVL2 ||
      mapId == MAP_RAVENS_POINT_LVL3) {
    return BotState::InDungeon;
  }

  if (mapId != MAP_OLAFSTEAD) {
    LogBot("Ravens: traveling to Olafstead from map %u", mapId);
    MapMgr::Travel(MAP_OLAFSTEAD);
    if (!DungeonNavigation::WaitForMapId(MAP_OLAFSTEAD, 60000u)) {
      return BotState::Error;
    }
    if (!WaitForMapReady(MAP_OLAFSTEAD)) {
  ...
  if (!WaitForMapReady(MAP_OLAFSTEAD, 5000u)) {
...
BotState HandleTravel(BotConfig &) {
  ...
  case MAP_OLAFSTEAD:
    ...
    return ExecuteRoute(RouteId::RunOlafsteadToVarajarFells, true)
               ? BotState::Traveling
               : BotState::Error;
  case MAP_VARAJAR_FELLS_1:
    if (!WaitForMapReady(MAP_VARAJAR_FELLS_1, 10000u)) {
    ...
    return BotState::InDungeon;
  case MAP_RAVENS_POINT_LVL1:
  case MAP_RAVENS_POINT_LVL2:
  case MAP_RAVENS_POINT_LVL3:
    return BotState::InDungeon;
  default:
...
void Register() {
  ...
  cfg.hard_mode = true;
  cfg.target_map_id = MAP_RAVENS_POINT_LVL1;
  cfg.outpost_map_id = MAP_OLAFSTEAD;
