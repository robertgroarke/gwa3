...
namespace GWA3::Bot::DungeonOutpostSetup {
...
std::size_t LoadHeroTemplatesFromFile(const char* filename,
    ...
    char path[MAX_PATH] = {};
    BuildRepoRelativePath("GWA Censured\\hero_configs", path, sizeof(path));
    const std::size_t used = std::strlen(path);
    if (used + 1u >= sizeof(path)) {
        return 0u;
    }
    if (used > 0u && path[used - 1u] != '\\') {
        std::strcat(path, "\\");
    }
    std::strcat(path, normalized_name);
...
bool ApplyOutpostSetup(BotConfig& cfg, const Options& options) {
    ...
    LogBot("Dungeon outpost setup using hero config: %s", cfg.hero_config_file.c_str());
    ...
    if (PartyMgr::CountPartyHeroes() > 0u) {
        LogBot("Clearing %u existing heroes before dungeon setup", PartyMgr::CountPartyHeroes());
        PartyMgr::KickAllHeroes();
        if (!WaitForHeroesCleared(options.clear_timeout_ms, options.clear_poll_interval_ms)) {
            LogBot("Dungeon outpost setup failed: heroes did not clear");
            return false;
        }
    }
    ...
    if (template_count > 0u) {
        for (std::size_t i = 0; i < template_count; ++i) {
            ...
            if (!WaitForHeroCount(static_cast<uint32_t>(i + 1u),
                ...
                LogBot("Dungeon outpost setup failed: hero %u did not join slot %zu",
                       templates[i].hero_id,
                       i + 1u);
                return false;
            }
        ...
        LogBot("Dungeon outpost setup falling back to preconfigured hero ids");
        for (std::size_t i = 0; i < fallback_hero_ids.size(); ++i) {
            ...
            if (!WaitForHeroCount(active_hero_count + 1u,
                                  options.add_hero_timeout_ms,
                                  options.clear_poll_interval_ms)) {
                LogBot("Dungeon outpost setup failed: fallback hero %u did not join", hero_id);
                return false;
            }
            ++active_hero_count;
            WaitMs(options.add_hero_delay_ms);
    ...
    if (active_hero_count == 0u) {
        LogBot("Dungeon outpost setup failed: no heroes configured");
        return false;
    }
    ...
    LogBot("Dungeon outpost setup complete: heroes=%u hardMode=%u",
           active_hero_count,
