...
bool MaybeAcquireBlessing(RouteId routeId) {
    ...
    const auto result = FroggyShared::TryAcquireBlessingAt(blessing->x, blessing->y, options);
    if (result.confirmed) {
        return true;
    }
...
bool ExecuteLevel1ExitDoorInteractSequence(const DoorOpenObjective& door) {
    if (!MoveToTravelPoint(door.interact_point, MAP_ARACHNIS_HAUNT_LVL1, 200.0f, 15000u)) {
        return false;
    }
    WaitMs(1000u);
...
bool ExecuteRewardChestFlow() {
    ...
    if (!MoveToTravelPointWithAggro(reward.chest_point, MAP_ARACHNIS_HAUNT_LVL2, 250.0f, 1200.0f, 60000u)) {
        return false;
    }
    ...
    return DungeonNavigation::WaitForMapId(MAP_MAGUS_STONES, 180000u);
}
...
bool ExecuteLevel1Phase1() {
    ...
    return ExecuteRoute(RouteId::Level1Phase1, "level 1 phase 1") &&
           PickUpObjectiveBundle(objectives[0], MAP_ARACHNIS_HAUNT_LVL1) &&
           ExecuteBundleClearPath(objectives[0], MAP_ARACHNIS_HAUNT_LVL1, true) &&
           ExecuteLevel1Phase2();
}
...
bool ExecuteLevel1Phase2() {
    ...
    return ExecuteRoute(RouteId::Level1Phase2, "level 1 phase 2") &&
           PickUpObjectiveBundle(objectives[1], MAP_ARACHNIS_HAUNT_LVL1) &&
           ExecuteBundleClearPath(objectives[1], MAP_ARACHNIS_HAUNT_LVL1, false) &&
           ExecuteLevel1Phase3();
}
...
bool ExecuteLevel1Phase3() {
    ...
    return ExecuteRoute(RouteId::Level1Phase3, "level 1 phase 3") &&
           ...
           ExecuteSpiderEggCluster(eggs[0], MAP_ARACHNIS_HAUNT_LVL1) &&
           ExecuteLevel1Phase4();
}
...
bool ExecuteLevel1Phase4() {
    ...
    return ExecuteRoute(RouteId::Level1Phase4, "level 1 phase 4") &&
           EnsureHeldBundleNearPlayer() &&
           ExecuteSpiderEggCluster(eggs[1], MAP_ARACHNIS_HAUNT_LVL1) &&
...
bool ExecuteLevel2Phase1() {
    ...
    return ExecuteRoute(RouteId::Level2Phase1Approach, "level 2 phase 1 pickup") &&
           PickUpObjectiveBundle(objectives[2], MAP_ARACHNIS_HAUNT_LVL2) &&
           ExecuteRoute(RouteId::Level2Phase1Transit, "level 2 phase 1 transit") &&
           ExecuteBundleClearPath(objectives[2], MAP_ARACHNIS_HAUNT_LVL2, true) &&
           ExecuteLevel2Phase2();
}
...
bool ExecuteLevel2Phase2() {
    ...
    return ExecuteRoute(RouteId::Level2Phase2, "level 2 phase 2") &&
           PickUpObjectiveBundle(objectives[3], MAP_ARACHNIS_HAUNT_LVL2) &&
           ExecuteBundleClearPath(objectives[3], MAP_ARACHNIS_HAUNT_LVL2, false) &&
           ExecuteLevel2Phase3();
}
...
bool ExecuteLevel2Phase3() {
    ...
    return ExecuteRoute(RouteId::Level2Phase3, "level 2 phase 3") &&
           EnsureHeldBundleNearPlayer() &&
           ExecuteSpiderEggCluster(eggs[2], MAP_ARACHNIS_HAUNT_LVL2) &&
           ExecuteSpiderEggCluster(eggs[3], MAP_ARACHNIS_HAUNT_LVL2) &&
           ExecuteSpiderEggCluster(eggs[4], MAP_ARACHNIS_HAUNT_LVL2) &&
           ExecuteSpiderEggCluster(eggs[5], MAP_ARACHNIS_HAUNT_LVL2) &&
...
BotState HandleTownSetup(BotConfig&) {
    ...
    if (mapId == MAP_RATA_SUM || mapId == MAP_MAGUS_STONES) {
        return BotState::Traveling;
    }
    if (mapId == MAP_ARACHNIS_HAUNT_LVL1 || mapId == MAP_ARACHNIS_HAUNT_LVL2) {
        return BotState::InDungeon;
    }

    LogBot("Arachnis: traveling to Rata Sum from map %u", mapId);
    MapMgr::Travel(MAP_RATA_SUM);
    if (!DungeonNavigation::WaitForMapId(MAP_RATA_SUM, 60000u)) {
        return BotState::Error;
    }
    if (!WaitForMapReady(MAP_RATA_SUM, 10000u)) {
        return BotState::Error;
    }
    return BotState::Traveling;
}
...
BotState HandleTravel(BotConfig&) {
    ...
    case MAP_RATA_SUM:
        ...
        return ExecuteRoute(RouteId::RunRataSumToMagusStones, "Rata Sum to Magus Stones", true)
                   ? BotState::Traveling
                   : BotState::Error;
    case MAP_MAGUS_STONES:
        if (!WaitForMapReady(MAP_MAGUS_STONES, 10000u)) {
            return BotState::Error;
        }
    ...
    case MAP_ARACHNIS_HAUNT_LVL1:
    case MAP_ARACHNIS_HAUNT_LVL2:
        return BotState::InDungeon;
    default:
...
void Register() {
    ...
    cfg.hard_mode = true;
    cfg.target_map_id = MAP_ARACHNIS_HAUNT_LVL1;
    cfg.outpost_map_id = MAP_RATA_SUM;
