...
; Stub function - the real implementation is in Utils-Storage-Bot.au3
; Returns True if the item is salvageable based on its properties
Func DefaultShouldSalvageItem($item)
	...
	; EXPLICITLY EXCLUDE KITS (ID Kits, Salvage Kits)
	; 5899 = Superior ID Kit (Verified cause of crash)
	; Add other kit IDs if known or use Type check if available
	If $itemID == 5899 Then Return False ; Superior Identification Kit
	If $itemID == 2992 Then Return False ; Identification Kit
	
	Local $itemType = DllStructGetData($item, 'Type')
	; Type 0 usually maps to 'Lead' or 'None' but for items?
	; Need to check GWA2 constants for Item Types if available
	
	; Check if item is salvageable according to game data
	If DllStructGetData($item, 'IsMaterialSalvageable') Then Return True
	
	; Check for salvageable trophies or other items that might not have the flag set correctly
	If IsTrophy($itemID) Then Return True
...
Func SalvageItems($buyKit = True)
	...
	If $kit == 0 Then 
		Out("Debug Salvage: No salvage kit found")
		Return False
	EndIf
	...
	For $bagIndex = 1 To _Min(4, $bags_count)
		Out("Debug Salvage: Checking bag " & $bagIndex)
		Local $bag = GetBag($bagIndex)
		If Not IsDllStruct($bag) Then ContinueLoop
		Local $bagSize = DllStructGetData($bag, 'slots')
		For $slot = 1 To $bagSize
			...
			If IsTrophy($itemID) Then
                ...
                EndIf
				If $trophyAndMaterialIndex < 60 Then
			...
			Else
				...
				If DefaultShouldSalvageItem($item) Then
					...
					EndIf
				Else
					Out("Debug Salvage: Item " & $itemID & " not salvageable")
				EndIf
			EndIf
		Next
	Next

	; Moving removed item back from Xunlai chest

	
	; Moved item processing
...
;~ Return true if the item is a Rare Skin (Green Item or Rare Skin weapon)
