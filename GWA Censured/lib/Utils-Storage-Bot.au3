...
Func UpdateNewUpgrades($upgradeType)
	...
	For $OSWeaponUpgradeType In $OSWeaponUpgradeTypes
		...
		$query = 'UPDATE ' & $TABLE_LOOKUP_UPGRADES & @CRLF _
			& "	SET hexa = '" & $modStruct & "' WHERE OS = " & $bananaSplit[1] & " AND upgrade_type = '" & $bananaSplit[2] & "' AND weapon = " & $bananaSplit[3] & " AND effect = '" & $bananaSplit[4] & "';"
		SQLExecute($query)
	Next
EndFunc
...
Func MoveItemsOutOfEquipmentBag()
	...
	For $slot = 1 To DllStructGetData($equipmentBag, 'slots')
		...
		If DllStructGetData($item, 'ModelID') <> 0 Then
			...
			If Not StorageBot_ShouldSalvageItem($item) Then ContinueLoop
		...
		For $i = 1 To DllStructGetData($bag, 'slots')
			...
			If $shouldSellMaterial($item) Then
				...
				While $totalAmount > 9
					...
					If $totalAmount >= $preQty Then 
						$failCount += 1
						Warn('Sell attempt failed (' & $failCount & '/3). Qty: ' & $totalAmount)
						If $failCount >= 3 Then ExitLoop
					Else
						$failCount = 0
					EndIf
				WEnd
...
Func StorageBot_ShouldSalvageItem($item)
	; Clarity rename
	Local $cache = $inventory_management_cache
