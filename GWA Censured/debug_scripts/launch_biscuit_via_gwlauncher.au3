...
Global Const $LOG_PATH = "C:\Users\Robert\Documents\GWA Censured X BotsHub\GWA Censured\debug_scripts\launch_biscuit_via_gwlauncher.log"
...
ConsoleWrite("=== Launch BISCUIT via GWLauncher ===" & @CRLF)
FileDelete($LOG_PATH)
FileWrite($LOG_PATH, "=== Launch BISCUIT via GWLauncher ===" & @CRLF)
...
If IsAtCharSelect() Then
    If IsReconnectDialogShowing() Then
        ConsoleWrite("Reconnect dialog detected; dismissing before selecting BISCUIT" & @CRLF)
