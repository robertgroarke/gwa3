...
;
; Research: Inject gwca.dll into a target GW client, verify initialization,
...
Global Const $TARGET_CHARACTER = "L I L B I S C U I T"
Global Const $REPORT_PATH = @ScriptDir & '\tests\gwca_inject_research_report.txt'

ConsoleWrite("=== GWCA Injection Research (" & $TARGET_CHARACTER & ") ===" & @CRLF)
ConsoleWrite("Time: " & @YEAR & "-" & @MON & "-" & @MDAY & " " & @HOUR & ":" & @MIN & ":" & @SEC & @CRLF)
FileDelete($REPORT_PATH)
FileWrite($REPORT_PATH, "=== GWCA Injection Research (" & $TARGET_CHARACTER & ") ===" & @CRLF)
FileWrite($REPORT_PATH, "Time: " & @YEAR & "-" & @MON & "-" & @MDAY & " " & @HOUR & ":" & @MIN & ":" & @SEC & @CRLF)

Func _AppendReport($text)
	FileWrite($REPORT_PATH, $text)
EndFunc

Func _DumpBytes($name, $processHandle, $addr, $len)
	ConsoleWrite(@CRLF & "=== " & $name & " @ 0x" & Hex($addr) & " ===" & @CRLF)
	_AppendReport(@CRLF & "=== " & $name & " @ 0x" & Hex($addr) & " ===" & @CRLF)
	...
	For $row = 0 To $len - 1 Step 16
		...
		For $b = 1 To 16
			If $row + $b <= $len Then
				$hex &= Hex(DllStructGetData($buf, 1, $row + $b), 2) & " "
			EndIf
		Next
		ConsoleWrite($hex & @CRLF)
		_AppendReport($hex & @CRLF)
	Next
EndFunc

; --- Step 0: Find or launch target client ---
Local $accounts = GWLauncher_LoadAccounts()
Local $accIdx = GWLauncher_FindAccountByCharacter($accounts, $TARGET_CHARACTER)
If $accIdx = -1 Then
	ConsoleWrite("ERROR: target account not found in Accounts.json" & @CRLF)
	Exit 1
EndIf

Local $targetGwPath = ""
If IsMap($accounts[$accIdx]) And MapExists($accounts[$accIdx], 'gwpath') Then
	$targetGwPath = $accounts[$accIdx]['gwpath']
EndIf

ScanAndUpdateGameClients()
Local $targetIdx = -1

; Check if the target client is already running
For $i = 1 To $game_clients[0][0]
	Local $title = $game_clients[$i][1]
	Local $pid = $game_clients[$i][0]
	Local $exePath = ""
	Local $hSnap0 = DllCall('kernel32.dll', 'handle', 'CreateToolhelp32Snapshot', 'dword', 0x8, 'dword', $pid)
	If IsArray($hSnap0) And $hSnap0[0] <> -1 Then
		Local $me0 = DllStructCreate('dword dwSize;dword th32ModuleID;dword th32ProcessID;dword GlblcntUsage;dword ProccntUsage;ptr modBaseAddr;dword modBaseSize;handle hModule;wchar szModule[256];wchar szExePath[260]')
		DllStructSetData($me0, 'dwSize', DllStructGetSize($me0))
		Local $r0 = DllCall('kernel32.dll', 'bool', 'Module32FirstW', 'handle', $hSnap0[0], 'struct*', $me0)
		If IsArray($r0) And $r0[0] Then $exePath = DllStructGetData($me0, 'szExePath')
		DllCall('kernel32.dll', 'bool', 'CloseHandle', 'handle', $hSnap0[0])
	EndIf

	ConsoleWrite("  Client " & $i & ": PID=" & $pid & " title='" & $title & "' path='" & $exePath & "'" & @CRLF)
	If StringInStr($title, $TARGET_CHARACTER) Or ($targetGwPath <> "" And StringLower($exePath) = StringLower($targetGwPath)) Then
		$targetIdx = $i
	EndIf
Next

If $targetIdx = -1 Then
	; Launch the target character
	ConsoleWrite($TARGET_CHARACTER & " not running, launching..." & @CRLF)
...
If $targetIdx = -1 Then
	...
	If $game_clients[0][0] > 0 Then
		$targetIdx = 1
		ConsoleWrite("WARNING: target client not found by name, using client 1" & @CRLF)
	Else
...
_AppendReport(@CRLF & "=== GWCA Data Section Dump ===" & @CRLF)
...
For $i = 0 To 9
	...
	_AppendReport("  " & $gwcaPtrs[$i][1] & " [+" & Hex($gwcaPtrs[$i][0]) & "]: 0x" & Hex($val) & @CRLF)
Next
...
_AppendReport(@CRLF & "=== Rendering Hook State ===" & @CRLF)
...
_AppendReport("QueueCounter: " & $queueCounter & @CRLF)
_AppendReport("QueueBase: 0x" & Hex($queueBase) & @CRLF)
_AppendReport("StatusCode: " & MemoryRead($processHandle, GetLabel('StatusCode'), 'dword') & @CRLF)
...
If IsArray($testShellcode) And $testShellcode[0] <> 0 Then
	...
	If $newCounter <> $queueCounter Then
		...
		_AppendReport("Rendering hook ACTIVE (counter " & $queueCounter & " -> " & $newCounter & ")" & @CRLF)
	Else
		ConsoleWrite("WARNING: Rendering hook may NOT be active (counter unchanged)" & @CRLF)
		_AppendReport("WARNING: Rendering hook may NOT be active (counter unchanged)" & @CRLF)
	EndIf
...
; --- Step 9: Raw frame-dispatch anchors ---
ConsoleWrite(@CRLF & "=== Raw Frame Dispatch Anchors ===" & @CRLF)
_AppendReport(@CRLF & "=== Raw Frame Dispatch Anchors ===" & @CRLF)
Local $gwImageBase = $pe_sections_ranges[0][0] - 0x1000
Local $gameSendFrame = Int(GetLabel('SendFrameUIMsg'))
Local $frameArrayLabel = Int(GetLabel('FrameArray'))
Local $frameArrayScan = 0
If IsDeclared('scan_results') Then
	If MapExists($scan_results, 'FrameArray') Then $frameArrayScan = $scan_results['FrameArray']
EndIf

ConsoleWrite("GW image base: 0x" & Hex($gwImageBase) & @CRLF)
ConsoleWrite("SendFrameUIMsg label: 0x" & Hex($gameSendFrame) & " (game+" & Hex($gameSendFrame - $gwImageBase) & ")" & @CRLF)
ConsoleWrite("FrameArray label: 0x" & Hex($frameArrayLabel) & " (game+" & Hex($frameArrayLabel - $gwImageBase) & ")" & @CRLF)
ConsoleWrite("FrameArray raw scan result: 0x" & Hex($frameArrayScan) & @CRLF)
_AppendReport("GW image base: 0x" & Hex($gwImageBase) & @CRLF)
_AppendReport("SendFrameUIMsg label: 0x" & Hex($gameSendFrame) & " (game+" & Hex($gameSendFrame - $gwImageBase) & ")" & @CRLF)
_AppendReport("FrameArray label: 0x" & Hex($frameArrayLabel) & " (game+" & Hex($frameArrayLabel - $gwImageBase) & ")" & @CRLF)
_AppendReport("FrameArray raw scan result: 0x" & Hex($frameArrayScan) & @CRLF)

_DumpBytes("Game SendFrameUIMsg", $processHandle, $gameSendFrame, 128)

If $frameArrayScan <> 0 Then
	Local $frameAnchor = $frameArrayScan - 0x40
	If $frameAnchor < $gwImageBase Then $frameAnchor = $frameArrayScan
	_DumpBytes("FrMsg.cpp anchor neighborhood", $processHandle, $frameAnchor, 128)
EndIf
...
_AppendReport(@CRLF & "GWCA module base saved to tests\\gwca_module_base.txt: 0x" & Hex($gwcaModule) & @CRLF)
_AppendReport("Screenshot: tests\\gwca_research_state.png" & @CRLF)

ConsoleWrite(@CRLF & "=== INJECTION RESEARCH COMPLETE ===" & @CRLF)
_AppendReport(@CRLF & "=== INJECTION RESEARCH COMPLETE ===" & @CRLF)
